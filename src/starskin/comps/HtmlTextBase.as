...
package starskin.comps
{
	...
	/**
	 * HTML 文本标签（替代 RichText）
	 * @author AoSnow
	 */
	public class HtmlTextBase extends UIComponent implements IDisplayText
	{
		//--------------------------------------------------------------------------
		//
		//  Class constants
		...
		protected function createTextField():void
		{
			...
			textField.addEventListener( "textChanged", textField_textModifiedHandler );
			textField.addEventListener( "widthChanged", textField_textFieldSizeChangeHandler );
			textField.addEventListener( "heightChanged", textField_textFieldSizeChangeHandler );
			textField.addEventListener( "textFormatChanged", textField_textFormatChangeHandler );
			addChild( textField );
		}
